...
const CONFIG = {
    ...
    // MULTI-API KEY SUPPORT (up to 10 keys per service)
    API_KEYS: {
        google: {
        ...
        bing: {
            keys: (process.env.BING_API_KEYS || process.env.BING_SEARCH_API_KEY || '').split(',').filter(k => k.trim()),
            currentIndex: 0,
...
class SearchEngines {
    ...
    async searchBing(query, state) {
        const { key } = apiKeyManager.getNextKey('bing');
        if (!key) {
        ...
        try {
            const { default: fetch } = await import('node-fetch');
            const url = `${CONFIG.API_KEYS.bing.endpoint}?q=${encodeURIComponent(query + ' ' + state + ' 2026')}&count=10`;
            ...
            if (data.error) {
                apiKeyManager.markKeyError('bing', key);
                throw new Error(data.error.message);
            }
            
            if (data.webPages?.value) {
                logger.info(`Bing: Found ${data.webPages.value.length} results for "${query}" in ${state}`);
    ...
    async searchAll(query, state) {
        const results = await Promise.all([
            this.searchGoogle(query, state),
            this.searchBing(query, state)
        ]);
